...
:- comment(title,"Agents").
:- comment(author,"Francisco Bueno").

:- comment(bug,"Currently, the agent has to be compiled explicitely as an
     active module. The same protocol than in the agent source code must be
     used for this. Automatic compilation is not working.").
:- comment(bug,"It seems that there are running-ahead problems with threads
     that prevent to correctly publish agent addresses, sometimes.").

:- comment(module,"An agent is an active module which has a main execution
...
:- comment(protocol/1,"A protocol is formed by a pair of modules which allow
...
:- comment(doinclude,module_address/2).
:- pred module_address(Agent,Address)
    # "(protocol defined) gives the @var{Address} of @var{Agent}.".

:- comment(doinclude,:: /2).
:- comment(doinclude,self/1).

:- comment(appendix,"This package is intended as a sample of how to program
