...
:- data        rowspawn/1.
:- inheritable rowspawn/1.

rowspawn('1').

:- export(set_rowspawn/1).

set_rowspawn(Rowspawn) :-
        atom(Rowspawn),
        set_fact(rowspawn(Rowspawn)),
        notify_changes.

:- export(get_rowspawn/1).

get_rowspawn(Rowspawn) :-
        rowspawn(Rowspawn).
...
:- data        columnspawn/1.
:- inheritable columnspawn/1.

columnspawn('1').

:- export(set_columnspawn/1).

set_columnspawn(Columnspawn) :-
        atom(Columnspawn),
        set_fact(columnspawn(Columnspawn)),
        notify_changes.

:- export(get_columnspawn/1).

get_columnspawn(Columnspawn) :-
        columnspawn(Columnspawn).
...
%        padx(X),
%        pady(Y).
creation_position([' ',min(row),R,min(rowspan),RS,min(column),C,min(columnspan),CS,min(padx),X,min(pady),Y]) :-
        column(C),
        columnspawn(CS),
        rowgrid(R),
        rowspawn(RS),
