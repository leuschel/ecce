...
BOOL CServerApp::InitInstance()
{
	...
	m_pServerTemplate = new CMultiDocTemplate(
		IDR_SERVERTYPE,
		RUNTIME_CLASS(CServerDoc),
		RUNTIME_CLASS(CMDIChildWnd), // custom MDI child frame
		RUNTIME_CLASS(CServerView));
	AddDocTemplate(m_pServerTemplate);
	...
	// Start the server
	UINT port = GetProfileInt("Settings", "Port", 2020);
	if (! m_DbServer.Start(port)) {
		AfxMessageBox(_T("Cannot start server"));
		return FALSE;
	}
