...
:- true pred is(X,+arithexpression) => num(X).
:- true comp is(X,Y) : (ground(Y),arithexpression(Y)) + eval.
:- true comp is/2 + (iso, native, sideff(free), bind_ins).
...
:- true comp (<)/2 + (iso, native, sideff(free), bind_ins).
