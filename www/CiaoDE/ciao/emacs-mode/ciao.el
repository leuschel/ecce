...
(defvar ciao-real-lib-dir "/home/asap/asap-online-demo/CiaoDE/ciao"
  "Where the actual Ciao lib directory is (and, thus, e.g., the image files).")

(defvar ciao-info-dir "/home/asap/asap-online-demo/CiaoDE/doc"
...
(defconst ciao-mode-version "1.13p0" 
...
(defcustom ciao-lpdoc-system (or (getenv "LPDOC") "lpdoc-2.0")
...
(defcustom ciao-lpdoc-libpath (or (getenv "LPDOCLIB") "/home/asap/asap-online-demo/CiaoDE/lpdoc/lib")
...
;;      "TOP-LEVEL/COMPILER"
     ...
     ["(Re)Load and check buffer into top level" ciao-load-and-check-buffer t]
...
;; 	      "icons/ciaopreprocsee" ciao-mode-map)
	     (tool-bar-add-item-from-menu 
	      'ciao-analyze-buffer "icons/ciaoanalysis" ciao-mode-map)
 	     (tool-bar-add-item-from-menu 
 	      'ciao-check-assertions "icons/checkassertions" ciao-mode-map)
 	     (tool-bar-add-item-from-menu 
 	      'ciao-optimize-buffer "icons/ciaopeval" ciao-mode-map)
	     (tool-bar-add-item-from-menu 
 	      'ciao-browse-preprocessor-options
 	      "icons/ciaocustomize" ciao-mode-map)
	     (tool-bar-add-item-from-menu 
...
;; (defcustom ciao-toolbar-icon-directory "/home/herme/Systems/CiaoDE/ciao/emacs-mode/clip/Systems/ciao/emacs-mode/icons"
...
;;      "GENERATE/VIEW DOCUMENTATION"
     ...
     ["Remove error (and dbg) marks in buffers"  ciao-unmark-last-run-errors t]
     ["Visit(/create) LPSETTINGS.pl file"        ciao-visit-lpdoc-settings t]
	...
	(if (is_a_menu new-line)
		...
		; QB: AB (2)
	      ...
	      (if (and (eq ciao-gm-recovering 0)
		       ...
		       (insert-string (ciao-remove-all-spaces default_opt))
...
(defun ciao-ciaopp-process-gmenu-just-write-answers (line-str)
 ...
 ; last thing we can expect (Note *other things* are notes or errors
	...
	(let ( (the_mark (match-end 3))
	  ...
	  (if (<= ciao-widget-id ciao-widget-last-changed-option)
	      (insert-string (ciao-remove-all-spaces 
...
(defun ciao-visit-lpdoc-settings ()
  "Visit, or create, the default @tt{LPSETTINGS.pl} file (which
controls all auto-documenter options)."
  (interactive)
  (let ((libsettings (concat ciao-lpdoc-libpath "/DEFAULTS.pl"))
	...
	(if (file-exists-p docsettings)
	  ...
	  (kill-line)
	  (beginning-of-buffer)
	  (search-forward "component := ")
	  (beginning-of-line)
	  (kill-line)
	  (beginning-of-buffer)
	  (search-forward "mainfile := ")
	  (kill-line)
	  (insert-string "\'")
	  ...
	  (insert-string "\'")
	  (insert-string ciao-lpdoc-libpath)
	  (insert-string "\'.")
	  (kill-line)
	  (beginning-of-buffer)
	  (save-buffer)
...
(defun ciao-gen-doc ()
  "Generate the documentation according to @tt{LPSETTINGS.pl} in the
default format. This allows generating complex documents but it
assumes that @tt{LPSETTINGS.pl} exists and that the options that it
...
@tt{LPSETTINGS.pl} and @tt{Makefile} files (see the LPdoc manual)."
  (interactive)
  ...
  (let ((thisfile (buffer-name (current-buffer))))
    (if (not (file-exists-p 
	      (concat (ciao-lpdoc-buffer-tmpdir thisfile) "/LPSETTINGS.pl")))
	(message 
	 "You need to first visit LPSETTINGS.pl and perhaps choose options")
...
buffer. Basically, it creates a simple, default @tt{LPSETTINGS.pl}
file, sets @tt{mainfile} in @tt{LPSETTINGS.pl} to the current buffer
...
documentation directory with the appropriate @tt{LPSETTINGS.pl} file
(see the LPdoc manual)."
  ...
  (let ((thisfile (buffer-name (current-buffer)))
	...
	(settings (concat (ciao-lpdoc-buffer-tmpdir 
			   (buffer-name (current-buffer))) "/LPSETTINGS.pl")))
...
(defun ciao-start-viewer ()
  ...
  (let ((thisfile (buffer-name (current-buffer)))
    ...
    (if (not (file-exists-p (concat (ciao-lpdoc-buffer-tmpdir thisfile) 
				    "/LPSETTINGS.pl")))
	(message "You need to first choose options in LPSETTINGS.pl")
      (cond
...
(defun ciao-insert-string-with-face (string face)
...
(defun ciao-create-widget-buffer-from-list (str-list)
 ...
 ;; This has to dissapear from here
...
(defun ciao-widget-example ()  
  ...
  ( let ((ciao-widget-list))
    ...
    (setq ciao-widget-list (cons "An Edit box:                     (on) ? " ciao-widget-list))
    (setq ciao-widget-list (cons "Select Menu Level:               [naive, expert] (naive) ?" ciao-widget-list))

    (setq ciao-widget-list (cons "Select Action Group:             [analyze, check_assertions, optimize]
                                  (analyze) ? " ciao-widget-list))
