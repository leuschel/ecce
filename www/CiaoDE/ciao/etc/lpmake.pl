...
make_toplevel(Args, ApplName) :-
	get_alias_path,
	catch(parse_args(Args,ApplName,options(false,[])), E,
	    handle_make_error(E)).
...
parse_args([H|Args],ApplName, options(_,C)) :- 
	is_help_option(H),
	!,
	parse_args(Args,ApplName,options(true,C)).
...
parse_args(['-d',NameValue|Args],ApplName,Options) :-
	...
	!,
	parse_args(Args,ApplName,Options).
parse_args([Type,File|Args],ApplName,options(H,ConfigFile)) :-
	(Type='-m', FFile = File ;Type='-l', FFile = library( File )),
	!,
	parse_args(Args, ApplName, options(H,[FFile|ConfigFile])).
parse_args(Args,ApplName,Options) :-
	parse_other_options(Args,ApplName,Options),
	!.
parse_args(Args,ApplName,_Options) :-
	throw(make_args_error("~n~w: Unrecognized Option -- ~w~nTry: `~w --help' for more information~n",
	   [ApplName,Args,ApplName],ApplName)).

parse_other_options(Args, ApplName, options(H, ConfigFiles)) :-
	(   ConfigFiles == [] ->
	    load_config_files( ['Makefile.pl'] , "(default) module" )
	;
...
lpmake [-v] [-d Name1=Value1] ... [-d Namen=Valuen] \\
  ...
  in the current directory as configuration file. The configuration
...
report_commands_aux(LoadedFile) :-
	...
	(  findall(Target,call_unknown(target_exists(Target)),Targets),
	   Targets = [_ |_ ]
	-> ( member(Target,Targets),
		 ...
		 true
	     ;
		 call_unknown(target_comment(Target, Comment, Args)) ->
