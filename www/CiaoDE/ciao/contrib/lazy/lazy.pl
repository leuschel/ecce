...
@begin{verbatim}
@includeverbatim{examples/module1.pl}
@end{verbatim}
...
:- comment(appendix, "The translation of the code in order to execute it
   ...
   functions package). Then, the resulting predicate is transformed to
   ...
   function is translated by our this package. The following code returns
...
@noindent
   ...
   definition are @tt{add/2}, defined as @tt{add(X,Y) := X + Y.}, and
   @tt{tail/2} and @tt{zipWith/3}. These last two functions can be found in
   the @bf{lazy_lib.pl} runtime file.
