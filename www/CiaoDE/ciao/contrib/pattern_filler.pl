:- module( pattern_filler , [ get_pattern/2
                            , complete_with_pattern/3
			    ],
%	_ ,
			    [ assertions , regtypes ] ).
...
:- comment( title , "Binary Operator Patterns Filler" ).

:- comment( module, "This module has been though to complete terms that are
...
%% These are the directories we check.  
%% Windows-based paths should also be added. 
base('/tmp').
base('/var/tmp'). 
base('/usr/tmp'). 
base('.').
...
check_writable_dir(FileSpec):-
        base(TMP),
        file_exists(TMP, 2),                    % '2' is for writing
...
:- true pred format_to_string(Format, Arguments, String) 
   : format_control * list * string
...
:- comment(bug, 
          "@pred{format_to_string/3} writes an intermediate file which is 
           immediately removed, if possible.  It is not very fast.  In case
           there are no permissions, writing is attempted through an internal 
           pipe, which may hang if the string is too long
           (this is O.S. dependant)."). 
