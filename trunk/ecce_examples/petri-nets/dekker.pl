...
prolog target :
forward : sat([X1,X2,s(X3),X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16,s(X17),X18,X19,X20,X21,X22])
backward : p([s(0),0,0,0,0,0,0,0,s(0),0,s(0),0,s(0),0,s(0),0,0,0,0,0,0,0])
...
prop([X1,X2,s(X3),X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16,s(X17),X18,X19,X20,X21,X22],unsafe).
prop([X1,s(X2),X3,X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16,X17,X18,X19,X20,X21,X22],trying1).
prop([X1,X2,s(X3),X4,X5,X6,X7,X8,X9,X10,X11,X12,X13,X14,X15,X16,X17,X18,X19,X20,X21,X22],satisfied1).
...
finite_state_model_check(decker_liveness,Formula) :- start(S),
     Formula = sat(S,ag(or(not(p(trying)),ef(p(satisfied1))))).

/* Parametric model checking tasks */
parametric_model_check(_,_) :- fail.
     
/* Infinite Model Checking Tasks */
infinite_model_check(_,_) :-
     fail.
