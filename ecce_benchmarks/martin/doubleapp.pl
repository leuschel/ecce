:- module(doubleapp, [double_app/5]).
:- ensure_loaded(local).


double_app(X, Y, Z, Res, trace(1, App1, App2)):-
        append(X, Y, Int, App1),
        append(Int, Z, Res, App2).
...
append(X, Y, Z, D, Mode, Trace):-
    (D >= 0 ->
     ...
     ;
          ...
          freeze(Level, append(X, Y, Z, Level, bounded, Trace))
    ).

append1([], X, X, _, _, 2).
append1([X|Xs], Y, [X|Zs], D, Mode, trace(3, App)):-
    D1 is D - 1,
